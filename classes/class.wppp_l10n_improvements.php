...
class WPPP_L10n_Improvements extends WPPP_L10n_Improvements_Skeleton {
	...
	function is_jit_available () {
		global $wp_version;
		return isset( self::$jit_versions[$wp_version] );
	}
	...
	public function early_init () {
		...
		if ( $this->is_jit_available() && $this->wppp->options['use_jit_localize'] ) {
			...
			if ( !isset( $wp_scripts ) && !defined( 'IFRAME_REQUEST' ) ) {
				global $wp_version;
				include( sprintf( "%s/jit-by-version/wp" . self::$jit_versions[$wp_version] . ".php", dirname( __FILE__ ) ) );
